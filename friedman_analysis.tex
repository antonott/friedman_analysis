...
iLet $f(z)$ be an entire complex analytic function and let $T \in \sc B(X)$. Prove that $[f(T)]^* = f(T^*)$.
...
\begin{solution}
  ...
  \end{equation*}
  so $(\lambda_0 I - T)$ has a right inverse $q(T) R(p(\lambda_0); p(T))$ and a left inverse $R(p(\lambda_0); p(T)) q(T)$. It follows that both inverses are equal, hence making a two-sided inverse, which clearly is bounded. Thus $\lambda_0 \in \rho(T)$, and the proof is complete.
\end{solution}
